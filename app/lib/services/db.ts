...
export async function importWords(rows: AddWordPayload[]): Promise<ImportResult> {
  ...
  for (const row of rows) {
    ...
    const { data: existing } = await supabase
      .from('words').select('id').eq('turkish', row.turkish.trim()).maybeSingle()
    ...
    const { error } = await supabase.from('words').insert({
      ...row, word_type: wordType, created_by: user?.id ?? null,
    })
    if (!error) added++
    else { skipped++; errors.push(`${row.turkish}: ${error.message}`) }
  }
